<div class="forum-actions btn-group">
	@if(Auth::user()->id == $topic->user_id)
		@if($topic->status == 'open')
			{{ Form::open(array('route' => array(get_route_prefix() .'forum.front.topic.resolve', $topic->forum->slug, $topic->slug))) }}
			<button title="Resolve {{{ $topic->title }}}" class="btn btn-mini btn-success forum-topic-resolvelink">
				Resolve
			</button>
			{{ Form::close() }}
		@else
			{{ Form::open(array('route' => array(get_route_prefix() .'forum.front.topic.reopen', $topic->forum->slug, $topic->slug))) }}
		...
		{{ Form::open(array('route' => array(get_route_prefix() .'forum.front.topic.delete', $topic->forum->slug, $topic->slug), 'method' => 'DELETE')) }}
